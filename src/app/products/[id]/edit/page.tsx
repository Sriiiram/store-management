...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    ]).then(([prodResult, catResult, supResult]) => {
      if (prodResult.status === "fulfilled") setProduct(prodResult.value);
  ...
  if (!product) {
    return (
      <div className="mx-auto max-w-4xl px-4 py-16 text-center">
        ...
        <Link href="/products" className="mt-4 inline-block text-sm text-primary-600 hover:text-primary-800">← Back to Products</Link>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-6">
        ...
        <h1 className="mt-2 text-2xl font-bold text-gray-900">Edit: {product.name}</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <Field label="Unit">
              <select name="unit" defaultValue={product.unit} className={inputClass}>
                {UNITS.map((u) => <option key={u} value={u}>{u}</option>)}
              </select>
            </Field>
            <Field label="GST Rate (%)">
              <select name="gstRate" defaultValue={product.gstRate} className={inputClass}>
                {GST_RATES.map((r) => <option key={r} value={r}>{r}%</option>)}
              </select>
            </Field>
            <div className="sm:col-span-2 lg:col-span-3">
              <Field label="Description">
                <textarea name="description" defaultValue={product.description ?? ""} className={textareaClass} rows={2} />
              </Field>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            <Field label="Cost Price (₹)">
              <input name="costPrice" type="number" step="0.01" min="0" defaultValue={product.costPrice} className={inputClass} />
            </Field>
            <Field label="MRP (₹)">
              <input name="mrp" type="number" step="0.01" min="0" defaultValue={product.mrp} className={inputClass} />
            </Field>
            <Field label="Selling Price — GST (₹)">
              <input name="sellingPriceGst" type="number" step="0.01" min="0" defaultValue={product.sellingPriceGst} className={inputClass} />
            </Field>
            <Field label="Selling Price — Cash (₹)">
              <input name="sellingPriceCash" type="number" step="0.01" min="0" defaultValue={product.sellingPriceCash} className={inputClass} />
            </Field>
            <Field label="Min Margin (%)">
              <input name="minMarginPct" type="number" step="0.1" min="0" defaultValue={product.minMarginPct} className={inputClass} />
            </Field>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            <Field label="Reorder Level">
              <input name="reorderLevel" type="number" min="0" defaultValue={product.reorderLevel} className={inputClass} />
            </Field>
            <Field label="Max Stock">
              <input name="maxStock" type="number" min="0" defaultValue={product.maxStock} className={inputClass} />
            </Field>
        ...
        <div className="rounded-xl border border-blue-200 bg-blue-50/50 p-6 shadow-sm">
          ...
          <p className="mt-2 text-sm text-gray-500">Help your staff sell better — describe what this product does and who it&apos;s for.</p>
          <div className="mt-5 grid gap-5 sm:grid-cols-2">
            <Field label="What It Does">
              <textarea name="whatItDoes" defaultValue={k?.whatItDoes ?? ""} className={textareaClass} rows={3} />
            </Field>
            <Field label="What It Can't Do">
              <textarea name="whatItCantDo" defaultValue={k?.whatItCantDo ?? ""} className={textareaClass} rows={3} />
            </Field>
            <Field label="Best For">
              <textarea name="bestFor" defaultValue={k?.bestFor ?? ""} className={textareaClass} rows={3} />
            </Field>
            <Field label="Not Suitable For">
              <textarea name="notSuitableFor" defaultValue={k?.notSuitableFor ?? ""} className={textareaClass} rows={3} />
            </Field>
            <Field label="Key Specs">
              <textarea name="keySpecs" defaultValue={k?.keySpecs ?? ""} className={textareaClass} rows={3} />
            </Field>
            <Field label="Common Questions">
              <textarea name="commonQuestions" defaultValue={k?.commonQuestions ?? ""} className={textareaClass} rows={3} />
            </Field>
            <Field label="Accessories Needed">
              <textarea name="accessories" defaultValue={k?.accessories ?? ""} className={textareaClass} rows={3} />
            </Field>
            <Field label="Alternatives">
              <textarea name="alternatives" defaultValue={k?.alternatives ?? ""} className={textareaClass} rows={3} />
            </Field>
            <div className="sm:col-span-2">
              <Field label="Pro Tips">
                <textarea name="proTips" defaultValue={k?.proTips ?? ""} className={textareaClass} rows={3} />
              </Field>
            </div>
          </div>
        </div>
